((Utils) => {
    const App = {
      htmlElements: {
        pokemonFinderForm: document.querySelector("#pokemon-finder-form"), // El formulario
        pokemonFinderSearchType: document.querySelector("#pokemon-finder-search-type"), // Drop list de la busqueda (Pokemon o habilidad)
        pokemonFinderInput: document.querySelector("#pokemon-finder-query"), // Lo que se esta buscando (El input)
        pokemonFinderOutput: document.querySelector("#pokemon-finder-response"),// Lo que se va a mostrar
      },
      init: () => { // Evento que inicializa la busqueda
      ...
      handlers: {
        pokemonFinderFormOnSubmit: async (e) => {
          ...
          const query = App.htmlElements.pokemonFinderInput.value; // Se intoduce el valor del input
          const searchType = App.htmlElements.pokemonFinderSearchType.value; // Se intoduce el valor del droplist
          ...
          try {
            ...
            if (species) {
              response['chain_evolves'] = await Utils.getEvolutionChain(species.url);
            }
            const renderedTemplate = App.templates.render({
              searchType,
              response,
            }); // Se crea la carta con la informacion que se va a mostrar
            App.htmlElements.pokemonFinderOutput.innerHTML = renderedTemplate; // Se retorna al HTML
          } catch (error) {
            App.htmlElements.pokemonFinderOutput.innerHTML = `<h1>${error}</h1>`; // Muestra cualquier error que ocurra
          }
        },
      },
      templates: {
        ...
        pokemonCard: ({ id, name, weight, height,abilities,sprites,chain_evolves }) => {
					const abilityList = abilities.map(
						({ ability, is_hidden }) =>
								`<li><a target="_blank">${ability.name}${ is_hidden ? `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
						...
						const evolvesList = chain_evolves.map(
						({ name, is_baby }) => 
								`<li>${name} ${ is_baby ? `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <table>
            <tr>
              <h1>${name} (${id})</h1>
                ...
                <td>
                  <h3>Weight/Heaight</h3>
                </td>
            </tr>

            <tr>
              <td>
                <img src="${sprites.front_default}" alt=""><img src="${sprites.back_default}">
              </td>
              <td>
                <p>${weight}/${height}</p>
              </td>
        ...
        abilityCard: ({ id, name, pokemon }) => {
          const pokemonList = pokemon.map(
            ({ pokemon, is_hidden }) =>
              `<li><a target="_blank" href="${pokemon.url}">${pokemon.name} ${is_hidden ? `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M12.975 2.475C12.675 2.175 12.15 2.175 11.775 2.475L9.975 4.275C9.1828 3.98092 8.34501 3.8286 7.5 3.825C4.65 3.9 2.1 5.4 0.75 7.875C0.9 8.175 1.125 8.475 1.35 8.775C1.95 9.6 2.7 10.275 3.525 10.8L2.25 12.075C1.95 12.375 1.875 12.9 2.25 13.275C2.55 13.575 3.075 13.65 3.45 13.275L12.975 3.675C13.275 3.3 13.275 2.775 12.975 2.475ZM5.025 9.225L4.05 10.2C3.15 9.675 2.325 8.925 1.725 8.025C2.625 6.75 3.825 5.85 5.25 5.4C4.275 6.45 4.2 8.1 5.025 9.225ZM7.575 6.75C7.2 6.375 7.275 5.775 7.65 5.4C8.025 5.1 8.55 5.1 8.925 5.4L7.575 6.75ZM13.725 7.125C13.35 6.6 12.9 6.075 12.375 5.7L11.625 6.45C12.225 6.9 12.75 7.425 13.2 8.1C11.925 10.05 9.75 11.25 7.425 11.25H6.825L6.075 12C6.6 11.925 7.05 12 7.5 12C9.975 12 12.3 10.8 13.725 8.775C13.95 8.475 14.1 8.175 14.325 7.875C14.1 7.65 13.95 7.35 13.725 7.125ZM10.5 7.5L7.5 10.5C9.15 10.5 10.5 9.15 10.5 7.5Z" fill="black"/>
              </svg>` : ""
              }</a></li>`
          );
